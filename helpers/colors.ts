...
export const colorToRgbaString = (color: string, delimiter: string = ' ') => {
	const rgba = colorToRgba(color)
	return `${rgba.r}${delimiter}${rgba.g}${delimiter}${rgba.b}${delimiter}${rgba.a}`
}

export const colorToRgba = (color: string) => {
	return hexToRgba(colorNameToHex(color) || color)
}
...
export const hexToRgba = (color: string) => {
	...
	if (hexWithoutHash.length === 3) {
		...
		// #rrggbb
		return {
			r: parseInt(hexWithoutHash[0] + hexWithoutHash[1], 16),
			g: parseInt(hexWithoutHash[2] + hexWithoutHash[3], 16),
			b: parseInt(hexWithoutHash[4] + hexWithoutHash[5], 16),
			a: 1,
		}
		...
		return {
			...
			g: parseInt(hexWithoutHash[2] + hexWithoutHash[3], 16),
			b: parseInt(hexWithoutHash[4] + hexWithoutHash[5], 16),
			a: parseInt(hexWithoutHash[6] + hexWithoutHash[7], 16),
		}
	} else {
